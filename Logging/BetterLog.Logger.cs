...
namespace Cookie.BetterLogging
{
    ...
    {
        private static LogNode GetLogFor(
            object obj,
            LogInfo info,
...
#else
            if (depth < 0)
                return new LogNode(AddPrefix(Serializer.Serialize(obj), prefix), info);
            switch (obj)
            {
                case string str:
        ...
        )
        {
            ...
            for (int i = 0; i < items.Count; i++)
                result.Add(GetLogFor(items[i], info, depth - 1, i.ToString()));

            return result.ToArray();
        }

        private static LogNode[] GetLogForDictionary(
        ...
        )
        {
            ...
            foreach (DictionaryEntry entry in dictionary)
                result.Add(
                    GetLogFor(entry.Value, info, depth - 1, Serializer.Serialize(entry.Key, 1))
                );
